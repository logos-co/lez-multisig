...
pub fn handle(
    ...
    // Multisig state unchanged, but must return account for every pre_state
...
mod tests {
    ...
    fn test_approve_adds_approval() {
        ...
        let (accounts_out, _) = handle(&accounts, 1);

        let proposal: Proposal = borsh::from_slice(&Vec::from(accounts_out[2].data.clone())).unwrap();
