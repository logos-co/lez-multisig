...
The ImageID is reproducible: `cargo risczero build --manifest-path methods/guest/Cargo.toml`, which builds in the `risczero/risc0-guest-builder:r0.1.91.1` image. The same guest bytes give the same id, and so the same PDAs.
